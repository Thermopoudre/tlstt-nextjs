    ...
    <section className="py-12 bg-[#111111]">
      <div className="container-custom">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Nos certifications et engagements qualité</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
          {displayLabels.map((label) => (
            ...
            >
              <div className="w-32 h-32 md:w-40 md:h-40 relative bg-[#1a1a1a] rounded-2xl shadow-lg p-4 border border-[#333] hover:border-[#3b9fd8] transition-all hover:shadow-xl">
                <Image
                  src={label.image_url}
                  alt={label.name}
                  fill
              ...
              <div className="mt-3 text-center max-w-[150px]">
                <h3 className="text-sm font-semibold text-white">{label.name}</h3>
              </div>
              
              {/* Tooltip */}
              {label.description && (
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-64 bg-[#3b9fd8] text-white text-xs p-3 rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 pointer-events-none">
                  {label.description}
