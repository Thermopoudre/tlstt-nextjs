...
export default function AdminMentionsLegalesPage() {
  ...
  const fetchContent = async () => {
    const { data } = await supabase.from('pages_content').select('*').eq('slug', 'mentions-legales').single()
  ...
  const handleSave = async () => {
    setSaving(true)
    setMessage(null)
    const { error } = await supabase.from('pages_content').upsert({ slug: 'mentions-legales', title: 'Mentions Légales', content, updated_at: new Date().toISOString() }, { onConflict: 'slug' })
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-3">
          <a href="/mentions-legales" target="_blank" className="btn-secondary flex items-center gap-2"><i className="fas fa-external-link"></i>Voir</a>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="mb-4">
          ...
          <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={25} className="input-field font-mono text-sm" placeholder="# Mentions Légales\n\n## Éditeur\n..." />
        </div>
      </div>
    </div>
  )
