...
export default function AdminMarketplacePage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'rejected':
        return <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-medium">Refusé</span>
      case 'sold':
        return <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">Vendu</span>
      default:
        return null
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <p className="text-2xl font-bold text-green-600">{listings.filter(l => l.status === 'approved').length}</p>
        </div>
        <div className="bg-white rounded-xl shadow p-4">
          <p className="text-sm text-gray-600">Vendues</p>
          <p className="text-2xl font-bold text-blue-600">{listings.filter(l => l.status === 'sold').length}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-lg p-12 text-center">
          ...
          <p className="text-gray-500 text-lg">
            {filter === 'pending' ? 'Aucune annonce en attente' : 'Aucune annonce'}
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Par {listing.seller_name} • {new Date(listing.created_at).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      {getStatusBadge(listing.status)}
                      <span className="text-xl font-bold text-primary">
                        {listing.type === 'don' ? 'Gratuit' : `${listing.price}€`}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className={`px-2 py-1 rounded text-xs ${
                        listing.type === 'vente' ? 'bg-blue-100 text-blue-800' :
                        listing.type === 'echange' ? 'bg-purple-100 text-purple-800' :
                        'bg-green-100 text-green-800'
                      }`}>
                        {listing.type === 'vente' ? 'Vente' : listing.type === 'echange' ? 'Échange' : 'Don'}
                      </span>
                    </div>
