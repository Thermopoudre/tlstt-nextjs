...
export default function AdminBoutiquePage() {
  ...
  const toggleActive = async (id: number, active: boolean) => {
    await supabase.from('shop_products').update({ active: !active }).eq('id', id)
    fetchProducts()
  }
            ...
            <div key={product.id} className="bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between pt-3 border-t">
                  <span className="text-sm">Stock: <span className={`font-medium ${product.stock === 0 ? 'text-red-600' : 'text-green-600'}`}>{product.stock}</span></span>
                  <div className="flex gap-2">
                    <button onClick={() => toggleActive(product.id, product.active)} className={`p-2 rounded-lg ${product.active ? 'text-green-600 hover:bg-green-50' : 'text-gray-400 hover:bg-gray-100'}`}>
                      <i className={`fas ${product.active ? 'fa-eye' : 'fa-eye-slash'}`}></i>
                    </button>
                    <Link href={`/admin/boutique/${product.id}`} className="p-2 text-gray-500 hover:text-primary hover:bg-gray-100 rounded-lg">
